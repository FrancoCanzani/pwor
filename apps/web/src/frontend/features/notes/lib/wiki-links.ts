...
export function normalizeNoteTitle(title: string | null | undefined): string {
...
export function resolveWikiLinkTarget(
  ...
  const needle = normalizeNoteTitle(target);
  if (!needle) return null;

  const exact = notes.find(
    (note) =>
      note.id !== currentNoteId &&
      normalizeNoteTitle(note.title) === needle,
  );
  ...
  if (needle === "untitled") {
    const untitled = notes.find(
      (note) =>
        note.id !== currentNoteId && normalizeNoteTitle(note.title) === "",
    );
...
export function filterNotesByQuery(
  ...
  const scored = pool
    .map((note) => {
      const title = normalizeNoteTitle(note.title) || "untitled";
      let score = 0;
      if (title === q) score = 3;
