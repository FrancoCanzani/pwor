  ...
  const buildFromImageData = (
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        lit.add(`${col},${row}`)
        // Image samples are thin; text in Arial Black is already solid. Thickening merges letterforms.
        if (p.src && col + 1 < cols) lit.add(`${col + 1},${row}`)
      }
    }
...
export function AsciiLogo({
  ...
  useEffect(() => {
    ...
    // Engine reads live options via setOptions; remounting would reset the grid.
  ...
  return (
    <div
      ...
      className={cn(
        ...
        interactive &&
          "cursor-pointer focus-visible:ring-1 focus-visible:ring-ring/50 focus-visible:outline-none",
        className,
      )}
    >
      <canvas
